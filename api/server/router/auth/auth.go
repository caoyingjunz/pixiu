...
func (a *authRouter) initRoutes(httpEngine *gin.Engine) {
	authGroup := &apiregistry.Group{
		...
		Entries: []apiregistry.RouteEntry{
			...
			{Method: "GET", RelativePath: "/oauth/providers", Handler: a.listOAuthProviders, Description: "查看第三方登录源", Persist: &persistPublicAuthAPI},
			...
			{Method: "GET", RelativePath: "/oauth/providers/:provider/login-url", Handler: a.getOAuthProviderLoginURL, Description: "获取第三方登录地址", Persist: &persistPublicAuthAPI},
			{Method: "POST", RelativePath: "/oauth/providers/:provider/login", Handler: a.loginWithOAuthProvider, Description: "第三方登录", Persist: &persistPublicAuthAPI},
		},
	}
