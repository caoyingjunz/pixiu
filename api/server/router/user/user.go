...
func (u *userRouter) initRoutes(httpEngine *gin.Engine) {
	userGroup := &apiregistry.Group{
		...
		Entries: []apiregistry.RouteEntry{
			...
			{Method: "GET", RelativePath: "/feishu/config", Handler: u.getFeishuOAuthConfig, Description: "查看飞书登录配置"},
			{Method: "PUT", RelativePath: "/feishu/config", Handler: u.updateFeishuOAuthConfig, Description: "更新飞书登录配置"},
			{Method: "GET", RelativePath: "/feishu/login-url", Handler: u.getFeishuLoginURL, Description: "获取飞书登录地址"},
			{Method: "POST", RelativePath: "/feishu/login", Handler: u.loginWithFeishu, Description: "飞书登录"},
