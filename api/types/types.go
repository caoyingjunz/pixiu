...
type LoadCloud struct {
	Name      string    `json:"name"`       // 名称，系统自动生成，只能为字符串
	AliasName string    `json:"alias_name"` // 可读性的名称，支持中文
	CloudType CloudType `json:"cloud_type"` // cloud 的类型，支持标准类型和自建类型
	RawData   []byte    `json:"raw_data"`   // kubeconfig 的原始数据
}
